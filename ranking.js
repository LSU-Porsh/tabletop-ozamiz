...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderLeaderboard() {
    ...
    get(rankingsRef).then((snapshot) => {
      ...
      if (data) {
        ...
        // Order rankings for casual play by total wins (descending).
        players.sort(
          (a, b) =>
            parseInt(b.wins || 0) - parseInt(a.wins || 0) ||
            b.rating - a.rating,
        );
        ...
        players.forEach((player, index) => {
          ...
          row.innerHTML = `
                        ...
                        <div class="last-played-col">${lastPlayed}</div>
  ...
  if (addPlayerForm) {
    addPlayerForm.addEventListener("submit", (e) => {
      ...
      const playerWins =
        parseInt(document.getElementById("playerWins").value) || 0;
      const playerLosses =
        parseInt(document.getElementById("playerLosses").value) || 0;
      const playerGames =
        parseInt(document.getElementById("playerGames")?.value) ||
        playerWins + playerLosses;
      const playerLastPlayed =
        document.getElementById("playerLastPlayed")?.value || null;

      const winRate = calculateWinRate(playerWins, playerLosses);
      // Default rating for casual players
      const rating = 1200;
  ...
  if (editPlayerForm) {
    editPlayerForm.addEventListener("submit", (e) => {
      ...
      const playerWins =
        parseInt(document.getElementById("editPlayerWins").value) || 0;
      const playerLosses =
        parseInt(document.getElementById("editPlayerLosses").value) || 0;
      const playerGames =
        parseInt(document.getElementById("editPlayerGames")?.value) ||
        playerWins + playerLosses;
      const playerLastPlayed =
        document.getElementById("editPlayerLastPlayed")?.value || null;
      // Preserve existing rating from hidden input
      const rating =
        parseInt(document.getElementById("editPlayerRatingHidden")?.value) ||
        1200;
      ...
      if (e.target.classList.contains("edit-btn")) {
        ...
        get(playerRef).then((snapshot) => {
          if (snapshot.exists()) {
            ...
            // preserve rating in hidden input
            if (document.getElementById("editPlayerRatingHidden")) {
              document.getElementById("editPlayerRatingHidden").value =
                player.rating || 1200;
            }
            if (document.getElementById("editPlayerGames")) {
              document.getElementById("editPlayerGames").value =
                player.games || player.wins + player.losses || 0;
            }
            if (document.getElementById("editPlayerLastPlayed")) {
              document.getElementById("editPlayerLastPlayed").value =
                player.lastPlayed || "";
            }
  ...
  function importDataFromFirebase(jsonData) {
    try {
      ...
      if (
        confirm(
          "Are you sure you want to overwrite current data? This cannot be undone.",
        )
      ) {
        set(rankingsRef, data)
          .then(() => {
  ...
  function clearDataFromFirebase() {
    if (
      confirm(
        "⚠️ Are you sure you want to clear all player data for this game? This action cannot be undone.",
      )
    ) {
      remove(rankingsRef)
        .then(() => {
  ...
  // =========== Modal Closing Logic ===========
  const modals = [adminLoginModal, playerModal, editPlayerModal].filter(
    (m) => m != null,
  );
